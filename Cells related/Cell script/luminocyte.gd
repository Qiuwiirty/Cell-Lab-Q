...
func simulate_step(delta: float) -> void:
	super(delta)
	$photocyte_detector/collision.scale = Vector2(gprop(Props.LUM_SCALE), gprop(Props.LUM_SCALE))
	$light.scale = Vector2(gprop(Props.LUM_SCALE), gprop(Props.LUM_SCALE))
	...
	$light.material.set_shader_parameter("intensity", gprop(Props.LUM_INTENSITY))
	var i = 0
	for photocyte in photocytes_in_light:
		if !is_instance_valid(photocyte):
			photocytes_in_light.remove_at(i)
			continue
		var distance_mod = (128 * gprop(Props.LUM_SCALE)) / global_position.distance_to(photocyte.global_position)
		var result = distance_mod * delta * gprop(Props.LUM_INTENSITY) / 5 
...
func metabolism(delta, modifier := 1.0):
	super(delta, modifier + (gprop(Props.LUM_INTENSITY) + gprop(Props.LUM_SCALE)) / 2.0)
